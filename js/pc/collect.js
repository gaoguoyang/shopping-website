$(document).ready(function(){
	...
	$(".Select").click(function(){
		$(this).find("b").eq(1).toggleClass("up_arrow");
	})
	...
	$(".twoGoods").click(function(event){
		...
		if(ePos >= FatherLeft+offsetWidth*2 && ePos <= FatherLeft+offsetWidth*3){
			$("#classBorder").stop().animate({
				left:offsetWidth*2
			},400)
		}
		// if(ePos >= FatherLeft+offsetWidth*3 && ePos <= FatherLeft+offsetWidth*4){
		// 	$("#classBorder").stop().animate({
		// 		left:offsetWidth*3
		// 	},400)
		// }
	})
	$(".Classify a").hover(function(event){
		var e = event||window.event;
		var offSetLeft = $(this)[0].offsetLeft;
		var offsetWidth = $(this)[0].offsetWidth;
		var FatherLeft = $("#body")[0].offsetLeft;
	...
	$(".rightPlus").click(function(event){
		...
		var determine = $(this).parent().parent().siblings(".checkInput").prop("checked");
		if(determine){
	...
	$(".checkInput").change(function(){
		var determine = $(this).prop("checked");
		var goodLength = $(".listGood").length;
		var checkBoxLength = $(".checkInput:checked").length;
		if(goodLength == checkBoxLength){
			$(".checkBoxQuan").find("input").prop("checked",true)
		}else{
			$(".checkBoxQuan").find("input").prop("checked",false)
		}
		if(determine){
			$(this).parent().find(".hover_show").css("display","block");
		}else{
			$(this).parent().find(".hover_show").css("display","");
		}
	})
	...
	$(".checkBoxQuan").change(function(){
		...
		if(determine){
			...
			$(".hover_show").css("display","block");
		}else{
			$(".checkInput").prop("checked",false);
			$(".hover_show").css("display","");
		}
	})

	$(".deleteGoods").click(function(){
		var determine = $(this).siblings(".checkInput").prop("checked");
